...
function createRegularStyles(className, width, height, background, sides, numSides) {
  var styles = {};
  styles[className] = {
  	position: "absolute",
  ...
  sides.map((ring, ringIdx) => {
    ring.map((side, sideIdx) => {
      styles["ring" + ringIdx + "side" + sideIdx] = {
        ...
        transform: "translateX(" + side[0][0] + "px) translateY(" + side[0][1] + "px) translateZ(" + side[0][2] + "px) rotateY(" + (-1 * (180 -(numSides-2) * 180 / numSides) * ringIdx)  + "deg) rotateX(" + (-1 * (180 -(numSides-2) * 180 / numSides) * sideIdx)  + "deg) "
      }
    })
  });
  return styles;
}
export default createRegularStyles
